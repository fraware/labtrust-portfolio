...
**Contribution / headline:** L0 replay with divergence localization; L1 stub and L1 twin; overhead and corpus coverage. For a compact, presentable snapshot of key numbers, see [RUN_RESULTS_SUMMARY.md](../datasets/runs/RUN_RESULTS_SUMMARY.md) (generated by run_paper_experiments.py).
...
**Contribution / headline:** Meta reduces collapse when non-vacuous; two real regimes with --fallback-adapter retry_heavy; auditability and PONR invariance; methodology and auditability when trigger is vacuous. For a compact, presentable snapshot of key numbers, see [RUN_RESULTS_SUMMARY.md](../datasets/runs/RUN_RESULTS_SUMMARY.md) (generated by run_paper_experiments.py).
