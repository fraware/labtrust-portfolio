...
**Contribution / headline:** Typed-plan firewall and expanded CPS-oriented suites with OWASP alignment; real-LLM table (Table 1b, pass_rate and Wilson CI); baseline 3-way (gated vs weak vs ungated); denial-trace statistics; containment only (no elimination claim). Latest camera-ready snapshot (`llm_eval_camera_ready_20260424`): synthetic red-team 15/15 pass, confusable 6/6 pass, jailbreak-style 4/4 pass, adapter latency p95 mean 36.70 ms, baseline denials gated/weak/ungated = 60/60/0, and OpenAI full-suite N=3 with gpt-4.1-mini and gpt-4.1 each 75/75 (CI95 [95.1, 100.0]). Supplementary robust package (`llm_eval_paper_bundle_final`) is verified and intentionally separate from canonical Table 1b; it is sourced from `llm_eval_robust_gpt_20260425_two_models_scratch` and must be cited with its own denominator and model set. Supplementary isolated GPT-5.x post-patch runs (not merged into canonical Table 1b): `gpt-5.4` 73/75 (97.3, CI95 [90.8, 99.3]) and `gpt-5.4-pro` 54/75 (72.0, CI95 [61.0, 80.9]). See [RUN_RESULTS_SUMMARY.md](../datasets/runs/RUN_RESULTS_SUMMARY.md).
...
**Result locations (canonical camera-ready bundle, committed):** under `datasets/runs/llm_eval_camera_ready_20260424/`. When regenerating locally, you may still use a scratch out dir such as `datasets/runs/llm_eval/` (gitignored by default) and then export into the canonical dir for freeze commits.
- `datasets/runs/p6_final_audit_20260424/` — final **engineering audit** bundle (recomputed summaries, hashes, GPT-5.x forensics, parser stress, replay frozen-vs-trace-scan, claims checklist); generate with `python scripts/export_p6_final_audit_bundle.py --out-dir datasets/runs/p6_final_audit_20260424` (read `README.md` there first).
- `datasets/runs/llm_eval_paper_bundle_final/` — verified supplementary robust package (multi-variant + stress, paper-ready gate for that package: `verify_p6_robust_gpt_bundle.py --schema full --require-paper-ready`); use for supplementary cross-model robustness reporting only.
- **Reviewer caveat:** If supplementary claims are not tied to `llm_eval_paper_bundle_final`, reviewers are correct to reject GPT-5.x/stress/prompt-variant/prompt-template claims as unsupported by canonical Table 1b lineage.
...
- `datasets/runs/llm_eval_camera_ready_20260424/confusable_deputy_results.json` — confusable deputy cases (6); run_manifest, attribution, denial_by_layer
- `datasets/runs/llm_eval_camera_ready_20260424/e2e_denial_trace.json` — claim and example: planner proposed unsafe, validator blocked, recovery safe
- `datasets/runs/llm_eval_camera_ready_20260424/adapter_latency.json` — when run with `--run-adapter` (default: 3 scenarios, 20 seeds); optional denial_stats when --denial-stats; optional latency_decomposition (validation_ms, capture_total_ms p50/p95/p99) when --latency-decomposition
- `datasets/runs/llm_eval_camera_ready_20260424/baseline_comparison.json` — when run with `--run-baseline` (gated vs weak vs ungated, tool-level unsafe)
- `datasets/runs/llm_eval_camera_ready_20260424/baseline_comparison_args.json` — when run with `--run-baseline --baseline-plan args_unsafe` (argument-level path-traversal; safe_args ablation)
- `datasets/runs/llm_eval_camera_ready_20260424/baseline_benign.json` — when run with `--run-baseline --baseline-plan benign` (benign_acceptance_rate, false_positive_count per mode)
- `datasets/runs/llm_eval_camera_ready_20260424/denial_trace_stats.json` — when run with `--run-adapter --denial-stats`
- `datasets/runs/llm_eval_camera_ready_20260424/p6_artifact_hashes.json` — from export_p6_artifact_hashes.py; artifacts SHA256 and reproducibility_table (table_id, artifact, model_id, timestamp_iso, seed, prompt_template_hash, evaluator_version, policy_version, artifact_hash)
- `datasets/runs/llm_eval/p6_failure_analysis.json` — from export_p6_failure_analysis.py (after real-LLM run with run_details); rows and by_case_mode for argument-level failure taxonomy (optional; may live under a supplementary run dir when regenerated)
...
- **Reproducibility:** All result JSONs include run_manifest with timestamp_iso, evaluator_version, policy_version; real-LLM adds prompt_template_hash. Run `export_p6_artifact_hashes.py` then `export_p6_reproducibility_table.py` for appendix table (result-to-artifact mapping). For submission freeze hygiene, run `export_p6_final_audit_bundle.py` (see `datasets/runs/p6_final_audit_20260424/`). Venue pack: papers/P6_LLMPlanning/sat-cps2026/EXPERIMENTS_RUNBOOK.md.
