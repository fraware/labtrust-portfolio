...
**Contribution / headline:** Feature-based and regression predictors beat global mean out-of-sample; leave-one-family-out eval; secondary targets for coordination tax and error amplification; exploratory scaling exponent (no power-law claim). Refreshed snapshot (12 seeds, fault-mix, seven scenarios): overall baseline MAE about 0.62 vs regression MAE about 0.08, mean PI coverage about 0.96, scaling exponent about 1.01 (R² about 0.89), trigger_met=true. Optional stricter OOS: `scaling_eval_family/heldout_results.json`. See [RUN_RESULTS_SUMMARY.md](../datasets/runs/RUN_RESULTS_SUMMARY.md).

**What it measures:** Can we predict tasks_completed (and derived proxies) for held-out scenarios or held-out taxonomy families? Global-mean vs feature-based vs regression baseline; MAE and 95% CI (including regression MAE CI across folds); `secondary_targets`; `scaling_fit` (exploratory exponent).

**Result location:** `datasets/runs/scaling_eval/heldout_results.json` (written by `scaling_heldout_eval.py` from multiscenario runs). Optional: `datasets/runs/scaling_eval_family/heldout_results.json` (`--holdout-mode family --no-secondary`).
