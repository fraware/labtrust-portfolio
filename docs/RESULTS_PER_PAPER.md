...
**Result locations:**
- `datasets/runs/e3_summary.json` — E3 replay-link summary over N seeds
...
- `datasets/releases/p0_e3_release/` — released run (after `produce_p0_e3_release.py` with release)
...
**What it measures:** L0 replay fidelity (thin-slice trace replays bit-identical); corpus of “trap” traces that are expected to diverge (four traps: nondeterminism, reorder, timestamp_reorder, hash_mismatch; discovery from *_trace.json); replay overhead; root cause category for divergences; overhead curve (p95 vs trace size). L1 = control-plane replay with recorded observations (trace-only).
...
- **L1:** See `bench/replay/README.md` and `kernel/trace/REPLAY_LEVELS.v0.1.md`: L1 = control-plane replay with recorded observations, not physics replay; third party can reproduce from trace + public spec only.
...
**What it measures:** tasks_completed and p95 latency under fault settings (no_drop, drop_005, delay_01, calibration_invalid_01, etc.) across scenarios; Centralized vs Blackboard adapter comparison; anti-gaming (always_deny, always_wait score poorly); adapter cost.

**Result locations:**
- `datasets/runs/maestro_fault_sweep/multi_sweep.json` — per-scenario, per-setting sweep (mean/stdev over seeds); fault_settings include lab-real `calibration_invalid_01`.
- `datasets/runs/maestro_fault_sweep/<scenario>/sweep.json` — per-scenario sweep
...
- `bench/maestro/adapter_costs.json` — loc_estimate, hours_estimate per adapter (Centralized, Blackboard).
- `bench/maestro/baseline_summary.json` — Centralized vs Blackboard table (and baseline_results.md)
...
- Baselines: same pipeline with different adapter; compare tasks_completed and p95_latency_ms per seed. For publishable tables use `--seeds 10`; see `docs/REPORTING_STANDARD.md`.
...
- `held_out_results[]`: per held-out scenario, `baseline_mae`, `feat_baseline_mae`, `regression_mae`, `actuals_mean`, `train_n`, `test_n`.
- `overall_baseline_mae`, `overall_feat_baseline_mae`, `overall_regression_mae`: aggregate; feature/regression beating global mean = “beat baseline out-of-sample.”
...
**What it measures:** Red-team: does the policy block disallowed tools and allow allowed ones? Confusable deputy: adversarial tool output (args requesting privilege) blocked. E2E denial trace: planner proposes unsafe, validator blocks, system recovers. Adapter latency: p95 and wall time for synthetic adapter runs. Position: typed plan firewall for any planner (LLM or heuristic).
...
**What it measures:** Mapping check (hazards → controls → evidence types; PONR coverage); per-scenario review (evidence_bundle, trace, PONR events, controls_covered); auditor walk-through (mapping + PONR + optional run review); Part 11-style audit trail alignment. Non-goals: no certification claim; translation layer only.
...
**What it measures:** Fixed (Centralized) vs meta-controller vs naive (switch-on-any-fault) baseline: tasks_completed_mean, collapse_count, regime_switch_count. Thrash control (hysteresis: require N consecutive fault observations before switch). Force-collapse sweep: which drop_prob yields collapse in some seeds. Effect size: `excellence_metrics.collapse_reduction_vs_fixed`, `difference_mean`, `difference_ci95`, `paired_t_p_value` (meta vs fixed comparison).

**Result locations:**
- `datasets/runs/meta_eval/comparison.json` — fixed, meta_controller, naive_switch_baseline; no_safety_regression, meta_reduces_collapse, collapse_definition, per_seed.
...
| P2 | REP-CPS = centralized on tasks; aggregation_variants, sybil_sweep, safety_gate_denial; robust lowers bias. |
| P3 | Fidelity pass; root_cause_category; witness_slice per divergence; overhead_curve; plot_replay_overhead; L1 = control-plane replay with recorded observations (not physics). |
| P4 | Fault sweep (incl. calibration_invalid_01); anti-gaming + scoring_proof; adapter_costs.json; plot_maestro_recovery. |
| P5 | Held-out MAE; per_scenario_baseline_mae; feature/regression; 95% CI; scaling_fit; success_criteria_met.trigger_met; --seeds 10, --fault-mix. |
| P6 | Red-team + confusable_deputy + e2e_denial_trace; typed plan firewall for any planner; adapter_latency with --run-adapter. |
| P7 | mapping_check + ponr_coverage_ok; audit_bundle.py (--release); Part 11 mechanical table; export_assurance_gsn; non-goals; K7. |
| P8 | fixed/meta/naive; comparison.json + collapse_sweep.json (run_manifests); trigger_met; no_safety_regression; export_meta_tables; plot_meta_collapse (run meta_collapse_sweep first). |
