...
| Institutional baseline integrity | Main-text baseline row is `lab_profile_v0` with mapped controls/PONR coverage and passing full review. | assurance_eval_aies/baseline_summary.json; baseline_table.tex |
| Portability integrity | Main-text portability row is `warehouse_v0` and is generated by the same scripted review path. | assurance_eval_aies/institutional_positive_summary.json; portability_table.tex |
| Bounded packet reproducibility | External reviewer packet is script-generated, path-redacted in submission mode, and checksum-tracked. | assurance_eval_aies/bounded_review_packet/packet_manifest.json; RUN_MANIFEST.json |
| Proxy demotion hygiene | Traffic/medical proxy artifacts are exported only in `proxy_stress_only` and excluded from default main-text exports. | assurance_eval_aies/proxy_stress_only/traffic_medical_proxy_summary.json |
