...
| P7 | assurance_eval/results.json, robust_results.json, negative_results.json (`by_scenario`, `by_perturbation`, lift metrics; submission-mode path redaction) | export_assurance_tables; export_p7_negative_tables (Tables 4–6 + reject matrix + lift + latency + by-scenario + boundary + redacted manifest); `papers/P7_StandardsMapping/generated_tables.md` (index) | export_p7_mapping_flow; `docs/figures/p7_review_stages.mmd` (Figure 2) | export_assurance_gsn (Figure 1) |
