...
Run `scripts/review_assurance_run.py <run_dir> [--pack path] [--scenario-id <id>]` to produce a machine-readable review outcome (evidence_bundle_ok, trace_ok, ponr_events, controls_covered, ponr_coverage, control_coverage_ratio). When `--scenario-id` is supplied, PONR events use kernel PONR task names from conformance (same as Tier 3 check); otherwise a heuristic (task_end or "disposition" in name) is used. Use output as "review outcome" attachment.
