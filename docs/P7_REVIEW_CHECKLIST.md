...
**Negative controls (suite):** `scripts/run_assurance_negative_eval.py` materializes deterministic invalid cases and writes `datasets/runs/assurance_eval/negative_results.json` (`by_scenario`, `by_perturbation`, lift metrics; run with `--submission-mode` for redacted manifests). `scripts/export_p7_negative_tables.py` emits manuscript CSVs including `p7_negative_by_scenario.csv` and `p7_boundary_case_summary.csv`. **`tests/test_assurance_negative_eval.py`** smoke-tests quick mode, known rejections, baseline false-accept presence, and submission-mode redaction.
