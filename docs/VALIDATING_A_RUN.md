...
The test job runs 64 unit and integration tests (including P1–P8 real-eval integration tests that run the eval script and assert on output; P8 adds meta-controller unit tests). It also runs the thin-slice pipeline, lab profile load, conformance check, and release-dataset step. The `conditional-evals` job runs per-paper evaluation scripts: P0 E3 (`produce_p0_e3_release.py --runs 10 --no-release`), P0 E2 (`e2_redaction_demo.py`; redacted trace is for audit only and is not replayed; evidence_bundle_redacted has replay_ok false), replay_eval, maestro_fault_sweep (--seeds 5), maestro_baselines, contracts_eval, run_assurance_eval, rep_cps_eval (--delay-sweep 0,0.05,0.1), generate_multiscenario_runs, scaling_heldout_eval, llm_redteam_eval, meta_eval --run-naive --fault-threshold 0. Results are written under `datasets/runs/` and `bench/maestro/`. See [EVALS_RUNBOOK.md](EVALS_RUNBOOK.md) and [STATE_OF_THE_ART_CRITERIA.md](STATE_OF_THE_ART_CRITERIA.md).
