...
- **Robust GPT audit bundle (no API):** `python scripts/run_robust_gpt_eval.py --materialize-only` (legacy materializer) or `python scripts/p6_robust_gpt_full_package.py --from-red-team <run>/red_team_results.json --out-dir datasets/runs/llm_eval_paper_bundle_final --run-id llm_eval_paper_bundle_final` (full package exporter). **One-shot verify (files + scoring audit):** `python scripts/verify_p6_robust_gpt_bundle.py --run-dir datasets/runs/llm_eval_paper_bundle_final --schema full --require-paper-ready --red-team-results datasets/runs/llm_eval_paper_bundle_final/red_team_results.json`. Validator-only stress labels: `python scripts/run_robust_gpt_eval.py --synthetic-stress-audit path.json`. Lower-level audit only: `python scripts/audit_llm_results.py --run-dir … [--red-team-results …] [--json]`.
...
# Robust full-package materialize + verify (supplementary freeze):
python scripts/p6_robust_gpt_full_package.py --from-red-team datasets/runs/llm_eval_robust_gpt_20260424_evalscratch/red_team_results.json --out-dir datasets/runs/llm_eval_paper_bundle_final --run-id llm_eval_paper_bundle_final
