...
- **Negative controls:** `scripts/run_assurance_negative_eval.py [--quick]` writes `datasets/runs/assurance_eval/negative_results.json` (`aggregate`, `by_mode`, `by_family`, `by_scenario`, `by_perturbation`, per-row codes and latency). Use `--submission-mode` (or `--redact-paths`) for blind-review-safe manifests. `scripts/export_p7_negative_tables.py` emits CSVs under `papers/P7_StandardsMapping/`: Tables 4–6 (`p7_negative_family_summary.csv`, `p7_ablation_summary.csv`, `p7_failure_reason_breakdown.csv`) plus `p7_perturbation_reject_matrix.csv`, `p7_aggregate_lift_metrics.csv`, `p7_latency_by_mode.csv`, `p7_negative_by_scenario.csv`, and `p7_boundary_case_summary.csv` when boundary cases are present.
