...
- **Location:** Cases are defined in code: `impl/src/labtrust_portfolio/llm_planning.py` (`RED_TEAM_CASES`, `CONFUSABLE_DEPUTY_CASES`).
- **Schema (per case):** `id`, `description`, `step` (tool/args), `expected_block` (bool).
- **How to add:** Add a new entry to the appropriate list in `llm_planning.py`; re-run `scripts/llm_redteam_eval.py`. No file discovery; counts are fixed per run.
- **Run manifest:** `run_manifest.red_team_case_count`, `run_manifest.red_team_cases` (in red_team_results.json); `run_manifest.confusable_deputy_case_count` (in confusable_deputy_results.json).
