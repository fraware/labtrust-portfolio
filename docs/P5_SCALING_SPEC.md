...
- **fault_setting_label:** Generator folder label (`no_drop`, `drop_005`, optional `calibration_invalid_01`, `delay_01` with `--fault-mix`). Publishable P5 uses `--fault-settings no_drop,drop_005` (narrow grid) unless you opt into full `--fault-mix`.
- **agent_count:** 1, 2, 4, 8 (default full grid when not using `--p5-lite`) or `--p5-lite` subset `[1, 4]`.
- **coordination_regime:** `centralized`, `hierarchical`, `blackboard`, `market`, `decentralized` (full grid when not using `--p5-lite`) or lite subset (`--p5-lite`: three regimes).
...
Default regression feature vector: `DEFAULT_FEATURE_COLS_P5` in `scaling.py` (OLS with Gauss-Jordan inverse for moderate `k`). When `len(feature_cols) >= 6`, `_ols_fit` adds a small **adaptive ridge** term to the diagonal of `X'X` for feature columns only (not the intercept), reducing ill-conditioning under leave-one-scenario-out splits where a held-out id leaves nearly singular designs or extrapolating test points.
...
**Triggers:** `success_criteria_met.trigger_met` requires regression to beat global, feature, and regime **admissible** baselines out-of-sample (`beat_*_out_of_sample` booleans). `beat_oracle_per_scenario_baseline` is informational. `negative_result` is true when admissible trigger fails.

**Strict protocol aggregation rule (required):** if any held-out fold has `regression_mae = null`, then protocol-level `overall_regression_mae` must be `null`, `trigger_met` must be `false`, and `negative_result` must be `true`, with explicit `regression_skipped_reason` and fold labels.

## Artifacts and scripts

| Path | Role |
|------|------|
| `scripts/generate_multiscenario_runs.py` | Sweeps scenarios × faults × (optional `--coordination-grid`) × `--clean`; optional `--fault-settings` to filter labels |
...
| `scripts/export_scaling_regime_agent_summary.py` | Frozen regime x agent empirical scaling summary (`scaling_summary/regime_agent_summary.json`) |
