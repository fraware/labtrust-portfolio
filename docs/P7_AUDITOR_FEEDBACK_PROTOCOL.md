...
1. **Run the scripted review:** From repo root with `PYTHONPATH=impl/src` and `LABTRUST_KERNEL_DIR=kernel`, run:
   ...
   - Optional: run the full **negative-control** suite (`scripts/run_assurance_negative_eval.py --submission-mode`) and inspect `negative_results.json` (including `generation` metadata) / `papers/P7_StandardsMapping/p7_perturbation_reject_matrix.csv` / `p7_negative_by_scenario.csv` / `p7_boundary_case_summary.csv` / `p7_generation_metadata.json` to see which injected faults are missed by weaker reviewer modes and where boundary limits appear ([P7_PERTURBATION_CHECKLIST.md](P7_PERTURBATION_CHECKLIST.md)).
