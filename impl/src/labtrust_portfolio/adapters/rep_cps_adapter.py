...
class REPCPSAdapter:
    ...
    def run(
        ...
        **fault_params: Any,
        ...
        # Keep MAESTRO_REPORT.v0.2 schema-compliant on the raw path.
        # Adapter-specific provenance is persisted in notes and trace metadata.
        report["notes"] = self._append_adapter_note(report.get("notes"))
    ...
    @staticmethod
    def _append_adapter_note(existing_notes: Any) -> str:
        suffix = " Adapter provenance: controller=rep_cps."
        if isinstance(existing_notes, str) and existing_notes.strip():
            if "controller=rep_cps" in existing_notes:
                return existing_notes
            return existing_notes + suffix
        return "MAESTRO_REPORT v0.2 generated by REP-CPS adapter." + suffix
