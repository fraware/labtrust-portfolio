#!/usr/bin/env python3
"""
Render P0 Mermaid figure sources to PNG and PDF next to docs/figures/*.mmd (camera-ready).
Requires `mmdc` on PATH (npm i -g @mermaid-js/mermaid-cli) or Node for npx.

Usage (repo root): python scripts/render_p0_mermaid_figures.py
"""
...
FILES = (
    "p0_assurance_pipeline.mmd",
    "p0_tier_lattice.mmd",
    "p0_redaction_figure.mmd",
)
...
def main() -> int:
    ...
    for name in FILES:
        ...
        for ext in ("png", "pdf"):
            ...
            if r.returncode != 0:
                print(f"Render failed ({name} -> {ext}): {r.stderr or r.stdout}", file=sys.stderr)
                failed = True
            else:
