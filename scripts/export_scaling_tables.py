...
def table3_recommend(rec: dict | None) -> list[str]:
    ...
    lines.extend(
        [
            f"| regime_match_rate | {_fmt(rec.get('regime_selection_accuracy'))} |",
            f"| mean_regret_tasks_completed | {_fmt(rec.get('mean_regret_tasks_completed'))} |",
            f"| regret_p95 | {_fmt(rec.get('regret_p95'))} |",
            f"| Brier collapse (LOFO rows) | {_fmt(rec.get('brier_collapse_on_test_rows'))} |",
            "",
        ],
    )
    return lines
...
def table4_family_agent(fam: dict | None, agent: dict | None) -> list[str]:
    ...
    if fam:
        ...
        lines.append(
            f"| Leave-one-family-out | {_fmt(fam.get('overall_regression_mae'))} | "
    ...
    if agent:
        ...
        lines.append(
            f"| Leave-one-agent-count-out | {_fmt(agent.get('overall_regression_mae'))} | "
            f"{sc.get('trigger_met')} |",
        )
    else:
...
def table6_calibration(data: dict) -> list[str]:
    ...
    lines.append(f"| paired_t_p_value (global vs feat fold MAE) | {_fmt(em.get('paired_t_p_value'))} |")
    lines.append("")
    return lines
...
def table7_sensitivity(sens: dict | None) -> list[str]:
    ...
    for cap, block in sorted(sens.get("by_max_seed", {}).items(), key=lambda kv: int(kv[0])):
        ...
        lines.append(
            f"| {cap} | {_fmt(block.get('overall_regression_mae'))} | {sc.get('trigger_met')} |",
        )
    lines.append("")
    return lines
...
def main() -> int:
    ...
    chunks: list[str] = []
    chunks.append(f"<!-- generated by export_scaling_tables.py; holdout_mode={main_data.get('holdout_mode')} -->")
    chunks.append("")
