#!/usr/bin/env python3
"""
Plot P1 scale_test throughput vs event count. Runs contracts_eval --scale-test at
multiple scale-events (e.g. 1k, 10k, 100k) and plots events_per_sec vs event count,
or reads existing scale_test.json and writes data for external plotting.
Usage: PYTHONPATH=impl/src python scripts/plot_contracts_scale.py [--out path]
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Plot P1 scale throughput vs event count")
    ap.add_argument(
        "--out",
        type=Path,
        default=DEFAULT_OUT,
    ...
    if not points:
        print("No scale_test data; run contracts_eval.py --scale-test first.")
        return 1
    ...
    try:
        ...
        ax.set_title("P1 Contract validator scale: throughput vs event count")
        plt.tight_layout()
