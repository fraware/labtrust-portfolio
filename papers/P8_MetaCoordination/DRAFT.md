...
**Key results.** (1) success_criteria_met.trigger_met and no_safety_regression in comparison.json (meta beats best fixed with no safety regression); (2) no_safety_regression: meta >= 90% of fixed tasks_completed; (3) meta_reduces_collapse when non-vacuous (collapse_count > 0 from sweep); (4) Excellence metrics: collapse reduction, switch_audit_trail_total, no_safety_regression, trigger_met in comparison.json; (5) regime_switch_count_total and switch audit trail in trace; (6) With --fallback-adapter retry_heavy: fallback_tasks_completed_mean vs fixed (two real regimes); publishable run uses run_paper_experiments.py --paper P8 (collapse_sweep then meta_eval --non-vacuous --fallback-adapter retry_heavy). When collapse_count = 0: present as methodology and auditability only. *Numbers from comparison.json and collapse_sweep.json. See [RUN_RESULTS_SUMMARY.md](../datasets/runs/RUN_RESULTS_SUMMARY.md).*
...
**Table 1 — Comparison (fixed vs meta vs naive).** Source: `datasets/runs/meta_eval/comparison.json`. Run `meta_eval.py --run-naive --fault-threshold 0` to regenerate; run manifest (seeds, scenario_id, fault_threshold) in comparison.json. Example (regime_stress_v0, fault_threshold=0, 20 seeds). If collapse_count is 0 under default settings, run the non-vacuous procedure below to obtain a meaningful comparison. *From non-vacuous run (collapse_count > 0) when available; see Reproducibility.*
