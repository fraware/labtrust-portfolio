...
**Canonical publishable timing (primary thin-slice trace, `--overhead-runs 20`, from `overhead_stats` in `summary.json`):** mean full L0 **0.0984** ms, p95 **0.1055** ms, p99 **0.124** ms; paired full-vs-apply-only mean difference **0.0901** ms (bootstrap 95% CI [0.0879, 0.0935] ms). Keep abstract, tables, and conclusion aligned to these values for the same run manifest. **L1** (twin stub and optional `--l1-twin`) is an **architectural extension**; primary results stay at L0 plus corpus statistics; when `--l1-twin` is used, `l1_twin_summary` includes multi-seed thin-slice stats **and** L1 runs on each passing `real_ingest` trace. We intentionally do **not** assert full-process record-replay determinism or plant/hardware behavioral equivalence under replay.
...
**Minimal run (under 20 min):** `python scripts/replay_eval.py --out datasets/runs/replay_eval/summary.json --overhead-curve --overhead-runs 5 --bootstrap-reps 100 --l1-twin` then `python scripts/export_p3_paper_figures.py` then `python scripts/verify_p3_replay_summary.py --strict-curve`.

**Publishable run:** `--overhead-runs 20`, `--thin-slice-seeds 42,43,44,45,46` (multi-seed sensitivity), `--bootstrap-reps 500` or higher, **`--l1-twin`** (multi-seed L1 + real_ingest L1 families), **`--overhead-curve`** (multi-point prefix curve from actual thin-slice length); `run_manifest` in summary.json. CI uses fewer replays and bootstrap reps; do not cite CI output as publishable tables.
...
1) `PYTHONPATH=impl/src python scripts/replay_eval.py --out datasets/runs/replay_eval/summary.json --overhead-curve --overhead-runs 20 --thin-slice-seeds 42,43,44,45,46 --bootstrap-reps 500 --l1-twin`
2) `python scripts/plot_replay_overhead.py` (refreshes `docs/figures/p3_replay_overhead.*` for verifier alignment)
3) `python scripts/export_replay_corpus_table.py --out-md papers/P3_Replay/generated_tables.md`
4) `PYTHONPATH=impl/src python scripts/export_p3_paper_figures.py`
5) `python scripts/verify_p3_replay_summary.py --strict-curve`
...
- **Figure 1 — Replay overhead vs trace size.** p95 replay time (ms) vs event count from `overhead_curve` (prefixes of the **primary** thin-slice trace). Points include bootstrap CIs when present. Prefix sizes scale with trace length (`scripts/replay_eval.py` emits multiple curve points for typical thin-slice lengths; `--strict-curve` requires at least two points).

**Key results.** (1) `fidelity_pass` true on thin-slice. (2) Corpus: `success_criteria_met.corpus_expected_outcomes_met` true when all expected outcomes hold (pass traces replay ok; traps diverge at expected seq); corpus categories (synthetic_trap, field_proxy, real_ingest, synthetic_pass) are reported per trace in Table 1b. (3) L1 stub: `l1_stub_ok` when twin config present. (4) L1 twin: publishable runs use `--l1-twin`, producing `l1_twin_summary` with multi-seed aggregate **and** `real_ingest_traces` (L1 on each passing real-ingest lane); see `l1_twin_evidence.md`. (5) Overhead: publishable defaults `--overhead-runs 20`, `--thin-slice-seeds 42,43,44,45,46`, `--bootstrap-reps` 500+, `--overhead-curve`.
