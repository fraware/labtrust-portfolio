...
\newcommand{\Lzero}{\ensuremath{\mathrm{L0}}}
...
Answer: full \Lzero{} is measurably slower than apply-only and final-hash-only, with paired full-vs-apply-only difference mean 0.0901\,ms (95\% bootstrap CI [0.0879, 0.0935]).  
...
\midrule
Apply-only, no hash & 0.0082 & 0.0000 & Lower bound; no verification, no audit \\
Final-hash-only & 0.0154 & 0.0072 & End-state verification only; no per-event localization \\
Full \Lzero{} replay & 0.0984 & 0.0901 & Per-event checks and structured diagnostics \\
\bottomrule
\end{tabular}
\end{table}
