...
The validator correctly blocks the canonical unsafe step; the real-LLM experiment tests whether models reliably emit unsafe argument forms under repeated prompting. \textbf{OpenAI (current repo snapshot):} full-suite mode, 10 runs/case, 25 cases/model (250 trials/model). \texttt{gpt-4.1-mini} and \texttt{gpt-4.1} each score 250/250 (100.0\%, 95\% Wilson CI [98.5, 100.0\%]). \textbf{Optional Prime matrix:} separate denominator and output directory; see venue \texttt{EXPERIMENTS\_RUNBOOK.md} and do not merge rows without relabeling.
