...
3. **Empirical credibility:** Every claim must be backed by a table, figure, or artifact already measured. **Primary evidence:** Table 1 (15 red-team + 6 confusable deputy + 4 jailbreak-style synthetic suite), Figure 1 (decision path), Block C (adapter latency + denial trace case study), Block D (tool-level and argument-level baselines). **Canonical real-LLM (Table 1b):** OpenAI `gpt-4.1-mini` and `gpt-4.1`, default `--real-llm-suite full`, publishable default 10 runs/case; report both case count and denominator from run_manifest. **Current repo snapshot:** N=10 runs/case, 25 cases/model, each model 250/250 (100.0%, Wilson [98.5, 100.0]). **Optional:** Prime Inference four-model matrix (different N and denominator) only if clearly labeled as a separate run; see EXPERIMENTS_RUNBOOK.md. **Extended experiments** (concurrency, storage, adaptive suite, etc.) require the matching `p6_*.json` artifact before they appear in the main text.
