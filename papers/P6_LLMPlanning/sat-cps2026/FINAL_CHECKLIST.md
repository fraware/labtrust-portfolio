...
- [ ] If Block B includes supplementary robust/GPT-5.x rows, the text labels them as a separate run family (not merged into canonical Table 1b denominator) and cites `datasets/runs/llm_eval_paper_bundle_final/` artifacts explicitly.
...
- [ ] If supplementary robust rows are cited, `python scripts/verify_p6_robust_gpt_bundle.py --run-dir datasets/runs/llm_eval_paper_bundle_final --schema full --require-paper-ready --red-team-results datasets/runs/llm_eval_paper_bundle_final/red_team_results.json` passes and the cited numbers come from that directory.
- [ ] `P6_CAMERA_READY_SUMMARY.json` exists in the canonical run and matches paper-facing numbers.
- [ ] **Final engineering audit bundle** is present at `datasets/runs/p6_final_audit_20260424/` (generated by `scripts/export_p6_final_audit_bundle.py`) and `FINAL_AUDIT_SUMMARY.md` reflects the current freeze sign-off.
- [ ] If the text cites **replay verification 60/60**, it explicitly scopes that statement to the frozen `replay_denials.json` summary (and does not imply no additional denied-step records exist in other committed `trace.json` files); see `p6_final_audit_20260424/reproducibility_check.json` (`replay_fresh_trace_scan`, `replay_frozen_vs_trace_scan`).
