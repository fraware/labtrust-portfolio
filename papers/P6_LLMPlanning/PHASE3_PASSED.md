...
- Repro under 20 min: Minimal run documented (adapter-seeds 3); run manifest in red_team_results.json and adapter_latency.json. Full publishable: --real-llm-runs 5, 3 scenarios, 20 seeds, --run-baseline (3-way), optional --baseline-plan args_unsafe and benign, optional --latency-decomposition.
- Variance: Scenarios and seeds in adapter_latency.json; red-team from eval; real-LLM 5 runs per case with pass_rate and Wilson CI (OpenAI canonical: gpt-4.1-mini, gpt-4.1).
