...
- **Table 1:** `python scripts/llm_redteam_eval.py --out datasets/runs/llm_eval_camera_ready_20260424` (output `datasets/runs/llm_eval_camera_ready_20260424/red_team_results.json`; contains success_criteria_met.red_team_all_pass, trigger_met). Use `datasets/runs/llm_eval/` only as local scratch.
- **Table 1b (Real-LLM):** Run with `--out datasets/runs/llm_eval_camera_ready_20260424 --real-llm --real-llm-runs 3 --real-llm-suite full` (default now includes red-team + confusable-deputy + jailbreak-style); per-case pass_rate_pct, 95% Wilson CI, latency mean ± stdev; overall pass_rate_pct and Wilson CI in run_manifest; export prints Table 1b.
...
`RED_TEAM_CASES`: 15 cases and `CONFUSABLE_DEPUTY_CASES`: 6 cases with expected_block (unsafe blocked, safe allowed), plus 4 jailbreak-style cases and adaptive JSON suites. Validator stack: allow_list + safe_args + ponr_gate (+ privilege heuristic); validators must block unsafe in red-team tests. OWASP LLM Top 10 aligned; containment posture only.
...
**Table 2 — Adapter latency (when --run-adapter).** Source: `datasets/runs/llm_eval_camera_ready_20260424/adapter_latency.json`. Units: tail_latency_p95_mean_ms (ms), task_latency_ms_p95 (ms), wall_sec (s). Run_manifest (adapter_scenarios, adapter_seeds, script) in adapter_latency.json. Produced when running `llm_redteam_eval.py --run-adapter`. To refresh: run with `--run-adapter` then `plot_llm_adapter_latency.py`.
...
**Key results.** (1) Red-team: 15/15 cases blocked/admitted as expected (success_criteria_met.red_team_all_pass, all_block_unsafe_pass); success_criteria_met.trigger_met=true. (2) Confusable deputy: 6/6 pass (confusable_deputy_results.json). (3) Jailbreak-style: 4/4 pass in `red_team_results.json.jailbreak_style`. (4) Adapter latency: tail_latency_p95_mean_ms=36.70 from adapter_latency.json over 3 scenarios x 20 seeds (publishable default). (5) Baseline comparison: 3-way tool-level from baseline_comparison.json with denial counts 60/60/0 and mean tasks_completed 3.95 for all modes (this denial-injection baseline does not replace required nominal steps). (6) **Latest real-LLM (OpenAI, Table 1b camera-ready run):** `--out datasets/runs/llm_eval_camera_ready_20260424 --real-llm-models gpt-4.1-mini,gpt-4.1 --real-llm-runs 3 --real-llm-suite full`; 25 cases per model (15 red-team + 6 confusable + 4 jailbreak), 75 trials per model; each model 75/75 (100.0%, Wilson [95.1, 100.0]). The real-LLM experiment verifies that, when two OpenAI models produce the targeted typed unsafe forms under the controlled full-suite prompt, the validator yields the expected allow/deny outcomes. (7) Argument-level baseline (safe_args ablation): baseline_comparison_args.json; gated 60, weak 0, ungated 0. (8) Benign utility: baseline_benign.json from `--baseline-plan benign`. (9) Denial-trace statistics: denial_trace_stats.json when --run-adapter --denial-stats. (10) Optional Prime matrix remains separate-output, separately labeled denominator. (11) Task-critical replacement/competition evidence is available in `task_critical_injection.json` (thin-slice harness). (12) `P6_CAMERA_READY_SUMMARY.json` consolidates paper-facing numbers for freeze hygiene. (13) **Supplementary isolated GPT-5.x post-patch runs (not merged into canonical Table 1b):** `gpt-5.4` 73/75 (97.3%, Wilson [90.8, 99.3]) and `gpt-5.4-pro` 54/75 (72.0%, Wilson [61.0, 80.9]) from `datasets/runs/llm_eval_openai_gpt54_postpatch_20260424` and `datasets/runs/llm_eval_openai_gpt54pro_postpatch2_n3_20260424`. *Regenerate with `python scripts/llm_redteam_eval.py` and `python scripts/run_paper_experiments.py --paper P6`. See [RUN_RESULTS_SUMMARY.md](../../datasets/runs/RUN_RESULTS_SUMMARY.md), [P6_RESULTS_REPORT.md](P6_RESULTS_REPORT.md), and `sat-cps2026/ENGINEERING_TRUTH_PACKAGE_2026-04-24.md`.*

**Reported run (3 scenarios, 20 seeds).** Adapter: tail_latency_p95_mean_ms = 36.70 ms (stdev 22.07 ms; 95% CI [31.12, 42.29]); 60/60 runs with denial when using denial-injection plan. Baseline: gated 60, weak 60, ungated 0 denials (tool-level); mean tasks_completed 3.95 (all). **Real-LLM (OpenAI, full suite, 3 runs/case):** gpt-4.1-mini and gpt-4.1 each 75/75 (100.0%, [95.1, 100.0]) over 25 cases/model. Optional Prime top-4 matrix (N=3): documented in venue runbook only if that artifact is regenerated and cited explicitly.

**Contribution to the literature.** LLM safety and tool-use benchmarks are often generic (prompt injection, jailbreak, output filtering) and not tied to a specific system or trace format. We contribute a **CPS-oriented typed-plan firewall** with (1) **deterministic capture** and traceability for audit and replay; (2) an expanded **red-team + confusable deputy + jailbreak-style + adaptive** suite aligned with OWASP LLM Top 10 (see [docs/P6_OWASP_MAPPING.md](../docs/P6_OWASP_MAPPING.md)); (3) **containment only**—we do not claim elimination of prompt injection or novel attacks; (4) **real-LLM evaluation** with configurable multi-run Wilson intervals (Table 1b, full-suite mode available by default); (5) **three-way baseline comparison** (gated vs weak vs ungated) and **denial-trace statistics** over multiple scenarios and seeds. The firewall integrates with MAESTRO so that blocked steps and latency are reported in the same artifact pipeline as other papers.
...
| Red-team | Expanded static+adaptive cases (15 red-team + 6 confusable deputy + 4 jailbreak-style + adaptive suite); tool-centric; expected_block | Often: adversarial prompts, jailbreak suites, multi-step attacks |
...
| Validator | allow_list + safe_args + ponr_gate + privilege heuristic | Varies: filters, guardrails, tool restrictions |

We run a scoped jailbreak-style and adaptive suite, but not open-ended adversarial campaign generation; scope remains tool-policy containment and deterministic capture for CPS integration. When real-LLM is run, Table 1b reports model_id, pass_rate, Wilson CI, and latency metrics for the configured suite.
...
- **Adversarial depth:** Red-team is still largely tool-centric and finite; not an open-ended autonomous attack search.
- **Validator stack:** Validator includes allow_list + safe_args + ponr_gate + privilege heuristic; these are containment gates over plan steps, not formal guarantees of complete attack coverage.
...
- **Real-LLM suite:** Multi-run (default 10) yields pass_rate_pct and Wilson CI per case; model output may still vary across providers/prompts; document pass rate and CI; synthetic table remains primary validator evidence.
...
**Reproducibility:** Validate plan: `validate_plan(typed_plan)`; policy: `policy_check_step(step, allowed_tools)`. Red-team: `llm_redteam_eval.py` writes `datasets/runs/llm_eval_camera_ready_20260424/red_team_results.json` (includes success_criteria_met.trigger_met, red_team_all_pass). With `--run-adapter`: writes `adapter_latency.json` (run_manifest: adapter_scenarios, adapter_seeds); then run `plot_llm_adapter_latency.py --latency datasets/runs/llm_eval_camera_ready_20260424/adapter_latency.json` to refresh Figure 1. Integration test: `tests/test_llm_p6.py` runs eval with --run-adapter and asserts both artifacts. Red-team cases: `llm_planning.RED_TEAM_CASES`. Adapter: `LLMPlanningAdapter`; trace metadata typed_plan_valid, typed_plan_captured. Spec: `kernel/llm_runtime/PLAN_SPEC.v0.1.md`.

**Submission note (P6).** Use a run where red_team_results.json has success_criteria_met.trigger_met true. With real-LLM (Table 1b): run with `--out datasets/runs/llm_eval_camera_ready_20260424 --real-llm --real-llm-models gpt-4.1-mini,gpt-4.1 --real-llm-runs 3 --real-llm-suite full` (or single model); report pass_rate_pct and 95% Wilson CI per case and overall; latency mean ± stdev; cite run_manifest fields (model_id, n_runs_per_case, suite_mode, prompt_template_hash, timestamp_iso, evaluator_version, policy_version). Synthetic red-team remains primary validator evidence. Baseline: report 3-way tool-level from baseline_comparison.json, argument-level from baseline_comparison_args.json, and benign false-positive study from baseline_benign.json when used. SaT-CPS venue checklist: sat-cps2026/FINAL_CHECKLIST.md.
...
```
For adapter latency decomposition, add `--latency-decomposition` to the `--run-adapter` command. For real-LLM Table 1b (OpenAI), use `--out datasets/runs/llm_eval_camera_ready_20260424 --real-llm --real-llm-models gpt-4.1-mini,gpt-4.1 --real-llm-runs 3 --real-llm-suite full` on the first command (requires API keys in `.env`). Optional Prime: separate `--out` directory and `--real-llm-provider prime`; see sat-cps2026/EXPERIMENTS_RUNBOOK.md.
