...
- **Table 1b (Real-LLM):** Run with `--real-llm --real-llm-runs 10 --real-llm-suite full` (default now includes red-team + confusable-deputy + jailbreak-style); per-case pass_rate_pct, 95% Wilson CI, latency mean ± stdev; overall pass_rate_pct and Wilson CI in run_manifest; export prints Table 1b.
...
**Key results.** (1) Red-team: 15/15 cases blocked/admitted as expected (success_criteria_met.red_team_all_pass, all_block_unsafe_pass); success_criteria_met.trigger_met=true. (2) Confusable deputy: 6/6 pass (confusable_deputy_results.json). (3) Jailbreak-style: 4/4 pass in `red_team_results.json.jailbreak_style`. (4) Adapter latency: tail_latency_p95_mean_ms=32.07 from adapter_latency.json over 3 scenarios x 20 seeds (publishable default). (5) Baseline comparison: 3-way tool-level from baseline_comparison.json with denial counts 60/60/0 and mean tasks_completed 3.95 for all modes. (6) **Latest real-LLM (OpenAI, Table 1b in this repo run):** `--real-llm-models gpt-4.1-mini,gpt-4.1 --real-llm-runs 10 --real-llm-suite full`; 25 cases per model (15 red-team + 6 confusable + 4 jailbreak), 250 trials per model; each model 250/250 (100.0%, Wilson [98.5, 100.0]). (7) Argument-level baseline (safe_args ablation): baseline_comparison_args.json; gated 60, weak 0, ungated 0. (8) Benign utility: baseline_benign.json from `--baseline-plan benign`. (9) Denial-trace statistics: denial_trace_stats.json when --run-adapter --denial-stats. (10) Optional Prime matrix remains separate-output, separately labeled denominator. *Regenerate with `python scripts/llm_redteam_eval.py` and `python scripts/run_paper_experiments.py --paper P6`. See [RUN_RESULTS_SUMMARY.md](../../datasets/runs/RUN_RESULTS_SUMMARY.md) and [P6_RESULTS_REPORT.md](P6_RESULTS_REPORT.md).*

**Reported run (3 scenarios, 20 seeds).** Adapter: tail_latency_p95_mean_ms = 32.07 ms (stdev 18.45 ms; 95% CI [27.41, 36.74]); 60/60 runs with denial when using denial-injection plan. Baseline: gated 60, weak 60, ungated 0 denials (tool-level); mean tasks_completed 3.95 (all). **Real-LLM (OpenAI, full suite, 10 runs/case):** gpt-4.1-mini and gpt-4.1 each 250/250 (100.0%, [98.5, 100.0]) over 25 cases/model. Optional Prime top-4 matrix (N=3): documented in venue runbook only if that artifact is regenerated and cited explicitly.
...
```
For adapter latency decomposition, add `--latency-decomposition` to the `--run-adapter` command. For real-LLM Table 1b (OpenAI), use `--real-llm --real-llm-models gpt-4.1-mini,gpt-4.1 --real-llm-runs 10 --real-llm-suite full` on the first command (requires API keys in `.env`). Optional Prime: separate `--out` directory and `--real-llm-provider prime`; see sat-cps2026/EXPERIMENTS_RUNBOOK.md.
