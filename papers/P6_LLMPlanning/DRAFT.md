...
**Reported run (3 scenarios, 20 seeds).** Adapter: tail_latency_p95_mean_ms = 32.07 ms (stdev 18.45 ms; 95% CI [27.41, 36.74]); 60/60 runs with denial when using denial-injection plan. Baseline: gated 60, weak 60, ungated 0 denials (tool-level); mean tasks_completed 3.95 (all). **Real-LLM (OpenAI, full suite, 3 runs/case):** gpt-4.1-mini and gpt-4.1 each 75/75 (100.0%, [95.1, 100.0]) over 25 cases/model. Optional Prime top-4 matrix (N=3): documented in venue runbook only if that artifact is regenerated and cited explicitly.
