...
**Key results.** (1) Adapter parity: REP-CPS vs Centralized tasks_completed equal (no throughput regression); success_criteria_met.adapter_parity true. (2) Aggregation under compromise: bias_robust < bias_naive; influence bound from aggregation_variants; excellence_metrics.bias_reduction_pct when bias_naive > 0. (3) trigger_met (success_criteria_met.trigger_met) when robust_beats_naive and adapter_parity; use run with --scenarios toy_lab_v0,lab_profile_v0 and 20 seeds so trigger is typically met. (4) Safety gate: protocol output does not actuate without policy check. (5) Excellence metrics (n>=2 seeds): difference_mean, difference_ci95, paired_t_p_value, power_post_hoc in summary.json. *Numbers from summary.json. Regenerate with rep_cps_eval.py. See [RUN_RESULTS_SUMMARY.md](../datasets/runs/RUN_RESULTS_SUMMARY.md).*
...
**Submission note (P2 trigger).** For submission, use a run with --scenarios toy_lab_v0,lab_profile_v0 and 20 seeds (e.g. run_paper_experiments.py --paper P2) so summary.json has success_criteria_met.trigger_met true and excellence_metrics (bias_reduction_pct, adapter_parity) populated. Verify trigger_met true for the run used for tables; state in the draft that the conditional trigger is met, or frame as conditional/optional per [CONDITIONAL_TRIGGERS.md](../docs/CONDITIONAL_TRIGGERS.md) (P2).
