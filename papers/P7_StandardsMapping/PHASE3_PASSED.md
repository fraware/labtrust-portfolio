...
- **Robust completion snapshot** (current committed run):
  - `mapping_check.ok` = true
  - `mapping_check.ponr_coverage_ok` = true
  - `aggregate.evidence_bundle_ok_rate` = 0.5
  - `aggregate.trace_ok_rate` = 0.5
  - `aggregate.review_pass_rate` = 0.5
  - `by_scenario.review_pass_rate`: `lab_profile_v0` = 1.0, `toy_lab_v0` = 1.0, `warehouse_v0` = 0.0, `traffic_v0` = 0.0
...
- **Discrimination (C4):** `negative_results.json` (includes `generation`, `by_scenario`, `by_perturbation`, lift metrics in `aggregate`) from `run_assurance_negative_eval.py`; Tables 4–6 plus `p7_perturbation_reject_matrix.csv`, `p7_aggregate_lift_metrics.csv`, `p7_latency_by_mode.csv`, `p7_negative_by_scenario.csv`, `p7_boundary_case_summary.csv`, `p7_submission_manifest_redacted.json`, and `p7_generation_metadata.json` from `export_p7_negative_tables.py`; `review_assurance_run.py --review-mode` documented; failure codes in `docs/P7_REVIEW_FAILURE_CODES.md`.
