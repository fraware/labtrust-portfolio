...
- **L1:** Control-plane replay with recorded observations. L1 = replay using the trace as the sole source of observations (no live simulator required). A third party can reproduce L1 from the trace and public spec alone; observations are those recorded in the trace (event payloads, timestamps, state hashes). Twin config validation stub implements the L1 contract (L0 + config); full twin replay is future work.
- **L2:** Hardware-assisted (aspirational).
